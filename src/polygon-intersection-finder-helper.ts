import { Point, LineSegment, Corner, Polygon, PolygonRecord, IEdge, ICorner, IIntersectionEdgePoint, IIntersectionCornerPoint, IntersectionPoint, Vector } from './polygon-intersection-finder-types';
export class PolygonIntersectionFinderHelper {
    ...
    public static compareBranches(backVector: Vector) {
        return function (a: { polygonId: number, edgeId: number, lineSegment: [number, number][] }, b: { polygonId: number, edgeId: number, lineSegment: [number, number][] }) {
            const branchAAngleToBack = PolygonIntersectionFinderHelper.findCounterClockwiseAngleBetweenVectors(backVector, PolygonIntersectionFinderHelper.findVectorBetweenPoints(a.lineSegment[0], a.lineSegment[1]) as [number, number]);
            const branchBAngleToBack = PolygonIntersectionFinderHelper.findCounterClockwiseAngleBetweenVectors(backVector, PolygonIntersectionFinderHelper.findVectorBetweenPoints(b.lineSegment[0], b.lineSegment[1]) as [number, number]);
